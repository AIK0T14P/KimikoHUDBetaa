...
-- Variables para mantener el estado de las características
...
local function CreateToggle(name, section, callback)
    ...
    local function Toggle()
        ...
        -- Guardar el estado de la característica
        ActiveFeatures[name] = Enabled
    end
...
local function Chams(enabled)
    ...
    local function createChams(player)
        ...
        local function applyChams(part)
            ...
            chamPart.Name = player.Name .. "Cham"Entiendo tus preocupaciones y las mejoras que deseas. Vamos a hacer algunos ajustes para solucionar estos problemas y mejorar la persistencia de las características activadas. Aquí tienes el código actualizado con las mejoras solicitadas:
...
local SettingsFeatures = {
    ...
    {name = "UITransparency", callback = function(value)
        MainBorder.BackgroundTransparency = value
        MainFrame.BackgroundTransparency = value
        Sidebar.BackgroundTransparency = value
        ContentFrame.BackgroundTransparency = value
    end, slider = true, min = 0, max = 1, default = 0.1}
}
...
-- Función para aplicar características activas
local function ApplyActiveFeatures()
    for name, value in pairs(ActiveFeatures) do
        if type(value) == "boolean" then
            local toggle = Sections[ActiveCategory]:FindFirstChild(name .. "Toggle")
            if toggle then
                toggle.SetState(value)
            end
        elseif type(value) == "number" then
            local slider = Sections[ActiveCategory]:FindFirstChild(name .. "Slider")
            if slider then
                slider.Value = value
            end
        end
...
LocalPlayer.CharacterAdded:Connect(function(newCharacter)
    ...
    -- Aplicar características activas después del respawn
    ApplyActiveFeatures()
end)
